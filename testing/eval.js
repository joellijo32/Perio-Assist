...
for (let p = 0; p < patients; p++) {
  const { transcript, truth } = JSON.parse(
    execFileSync('python3', ['testing/eval_gen.py', String(seed + p), String(teeth), '1'], {
      encoding: 'utf8',
  ...
  for (const chunk of transcript.toLowerCase().split('.')) {
    if (chunk.trim()) {
      const r = parseInto(state, chunk);
      if (latency) parseMs.push(r.ms);
    }
  }
...
if (latency) {
  const sorted = [...parseMs].sort((a, b) => a - b);
  const p = (q) => sorted[Math.min(sorted.length - 1, Math.floor(q * sorted.length))];
  const avg = sorted.reduce((s, v) => s + v, 0) / sorted.length;
  const budget = 5; // ms, per eval_metrics.md Tier 3
  console.log(
    `latency n=${sorted.length} avg=${avg.toFixed(2)}ms p50=${p(0.5).toFixed(2)}ms p95=${p(0.95).toFixed(2)}ms max=${sorted.at(-1).toFixed(2)}ms` +
      (p(0.95) > budget ? `  OVER BUDGET (${budget}ms)` : `  within ${budget}ms budget`),
  );
}
