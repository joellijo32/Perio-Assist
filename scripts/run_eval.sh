...
run "[1/3] parser unit tests"   node "$ROOT/src/perio.test.js"
run "[2/3] narrative TC suite"  node "$ROOT/scripts/cases.js"
run "[3/3] synthetic chart eval (seed=1, 10 patients, 16 teeth)" \
    node "$ROOT/scripts/eval.js" 1 10 16
...
echo "    python3 scripts/acoustic-eval/synth.py"
echo "    python3 scripts/acoustic-eval/run.py"
echo "    node   scripts/acoustic-eval/chartmatch.js"
